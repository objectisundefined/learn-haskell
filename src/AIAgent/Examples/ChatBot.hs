...
data ChatMessage = ChatMessage
  { msgContent   :: Text
  , msgRole      :: Text  -- "user", "assistant", or "system"
...
instance ToJSON ChatMessage where
  toJSON msg = object
    ...
    , "metadata"  .= Object (KM.fromList
        [ (fromText k, v) | (k, v) <- HM.toList (msgMetadata msg) ])
    ]
...
mkSimpleChatBot name description =
  let agent = mkStatefulAgent name [TextGeneration, Communication] $ \_memory input -> do
...
mkToolEnabledChatBot name tools =
  let agent = mkStatefulAgent name [TextGeneration, Communication, ToolUsage] $ \_memory input -> do
        case HM.lookup "message" input of
          Just (String userMessage) -> do
  ...
  where
    ...
    handleMathRequest msg =
      if "+" `Text.isInfixOf` msg
        ...
        else return $ Right $ object ["response" .= ("I can help with math, but I need clearer instructions." :: Text)]

-- | Start a new chat session
startChatSession :: ChatBot -> IO ChatSession
startChatSession bot = do
  sid <- fmap (Text.pack . show) getCurrentTime
  ...
  return $ ChatSession sid [] memory True

-- | Send a message to the chatbot and return the response with updated session
      ...
      case result of
        ...
        Right responseValue ->
          case responseValue of
            Object obj -> case KM.lookup (fromText "response") obj of
...
calculatorChatBot =
  let mathToolList = map funcTool mathTools
      agent = mkStatefulAgent "CalculatorBot" [TextGeneration, Computation, ToolUsage] $ \_memory input -> do
        case HM.lookup "message" input of
          Just (String userMessage) -> do
  ...
  where
    ...
    extractNumbers txt =
      let ws = Text.words txt
          numberWords = filter (Text.all (\c -> c `elem` ("0123456789.-" :: String))) ws
...
fileAssistantBot =
  let fileToolList = map funcTool fileTools
      agent = mkStatefulAgent "FileAssistant" [TextGeneration, DataRetrieval, ToolUsage] $ \_memory input -> do
        case HM.lookup "message" input of
          Just (String userMessage) -> do
            let lowerMsg = Text.toLower userMessage
